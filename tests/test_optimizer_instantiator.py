...
class TestOptimizerInstantiator(unittest.TestCase):
    def test_scheduler_builder(self):
        ...
        self.assertEqual(0.0, lr_scheduler.alpha)
        self.assertEqual(0.0, wd_scheduler.alpha)

        lr_scheduler, wd_scheduler = build_scheduler('cd: 0.1 alpha', learning_rate, weight_decay, training_steps_per_epoch, epochs)
        self.assertEqual(0.1, lr_scheduler.alpha)
        self.assertEqual(0.1, wd_scheduler.alpha)
        ...
        self.assertEqual(0.0, lr_scheduler.alpha)
        self.assertEqual(2.0, lr_scheduler._t_mul)
        self.assertEqual(1.0, lr_scheduler._m_mul)
        self.assertEqual(0.0, wd_scheduler.alpha)
        self.assertEqual(2.0, wd_scheduler._t_mul)
        self.assertEqual(1.0, wd_scheduler._m_mul)

        lr_scheduler, wd_scheduler = build_scheduler('cdr: 2 period, 3 t_mul, 0.9 m_mul, 0.1 alpha', learning_rate, weight_decay,
                                                     training_steps_per_epoch, epochs)
        self.assertIsInstance(lr_scheduler, CosineDecayRestarts)
        self.assertIsInstance(wd_scheduler, CosineDecayRestarts)
        self.assertEqual(0.1, lr_scheduler.alpha)
        self.assertEqual(3.0, lr_scheduler._t_mul)
        self.assertEqual(0.9, lr_scheduler._m_mul)
        self.assertEqual(0.1, wd_scheduler.alpha)
        self.assertEqual(3.0, wd_scheduler._t_mul)
        self.assertEqual(0.9, wd_scheduler._m_mul)
