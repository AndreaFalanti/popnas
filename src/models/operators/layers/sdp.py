...
class ScheduledDropPath(Layer):
    ...
    But these versions actually differ from the one used in FractalNet, since they don't check that at least one path survives!
    This leads to awful accuracies also on the training set, since a cell/block could output a totally 0-ed tensor.
    ...
    Output tensors that survive the drop path are scaled by factor (n_inputs / n_survived_inputs).
    '''
    def __init__(self, keep_probability: float, cell_ratio: float, total_training_steps: int, dims: int, name='scheduled_drop_path', **kwargs):
        ...
        # the number of times the weights are updated (batches_per_epoch * epochs)
        self.total_training_steps = total_training_steps
        # keep track of the current execution step
        ...
        # avoid problems when using mixed precision, assigning the correct dtype
        self.min_prob = tf.constant(0.0, dtype=mixed_precision.global_policy().compute_dtype)
    ...
    def call(self, inputs, training=None, mask=None):
        if training and self.keep_probability < 1.0:
            ...
            # make a noise shape with the same dimensionality of input.
            # The first dim is equal to the batch size, others are equal to 1 (single value for each batch sample).
            noise_shape = [tf.shape(input=inputs[0])[0]] + [1] * self.dims
            input_dtype = inputs[0].dtype

            # A mask for each input tensor, if =0 then the path is dropped
            binary_tensors = []
            ...
            ensure_path_tensor = tf.maximum(self.min_prob, 1 - agg_mask_sum)
            ...
            # new rescaling factor: n_inputs / n_survived_inputs, no rescaling when no inputs are dropped
