...
class SqueezeExcitationOpAllocator(BaseOpAllocator):
    ...
    def generate_depth_adaptation_layer(self, match: re.Match, filters: int, name_prefix: str = 'D/', name_suffix: str = '') -> Layer:
        ...
        pointwise_conv = Convolution(filters, ones, ones, weight_reg=self.weight_reg)
        se_layer = self.generate_normal_layer(match, filters)
    ...
    def generate_reduction_layer(self, match: re.Match, filters: int, strides: 'tuple[int, ...]',
        ...
        pool_size = tuple([2] * self.op_dims)
        max_pool = Pooling('max', pool_size, pool_size)
        max_pool_pw = PoolingConv(max_pool, filters, self.weight_reg)
        se_layer = self.generate_normal_layer(match, filters)
