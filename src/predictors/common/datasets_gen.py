...
def __prepare_rnn_inputs_2i(state_space: StateSpace, cell_spec: list):
    '''
...
def __prepare_rnn_inputs(state_space: StateSpace, cell_spec: list):
    ...
    '''
    # use categorical encoding for both input and operators, since LSTM works with categorical
    return state_space.encode_cell_spec(cell_spec, flatten=False)


def __generate_equivalent_cells_and_rewards(state_space: StateSpace, cell_specs: 'list[list]', rewards: 'list[float]' = None):
    ...
    for cell_spec, reward in zip(cell_specs, rewards):
        ...
        eqv_cells = [cell_spec + [(None, None, None, None)] * (state_space.B - len(cell_spec)) for cell_spec in eqv_cells]
...
def __preprocess_cell_specs_and_rewards(state_space: StateSpace, cell_specs: 'list[list]', rewards: 'list[float]', use_data_augmentation):
    ...
    if use_data_augmentation and rewards is not None:
        cell_specs, rewards = __generate_equivalent_cells_and_rewards(state_space, cell_specs, rewards)
    else:
        # add padding with None blocks, if cells have not the maximum capacity of blocks
        cell_specs = [cell_spec + [(None, None, None, None)] * (state_space.B - len(cell_spec)) for cell_spec in cell_specs]
...
def build_temporal_series_dataset_2i(state_space: StateSpace, cell_specs: 'list[list]', rewards: 'list[float]' = None,
    ...
    # data augmentation is used only in training (rewards are given), if the respective flag is set.
    # if data augment is performed, the cell_specs and rewards parameters are replaced with their augmented counterpart.
    cell_specs, rewards = __preprocess_cell_specs_and_rewards(state_space, cell_specs, rewards, use_data_augmentation)

    rnn_inputs = list(map(lambda cell: __prepare_rnn_inputs_2i(state_space, cell), cell_specs))
...
def build_temporal_series_dataset(state_space: StateSpace, cell_specs: 'list[list]', rewards: 'list[float]' = None,
                                     use_data_augmentation: bool = True):
    '''
    ...
    cell_series = list(map(lambda cell: __prepare_rnn_inputs(state_space, cell), cell_specs))
