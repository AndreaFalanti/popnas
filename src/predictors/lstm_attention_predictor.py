...
class AttentionLSTMPredictor(KerasPredictor):
    def __init__(self, search_space: SearchSpace, y_col: str, y_domain: 'tuple[float, float]',
        ...
        if name is None:
            name = f'AttentionLSTM_{"default" if hp_config is None else alternative_dict_to_string(hp_config)}_{"tune" if hp_tuning else "manual"}'
    ...
    def _build_model(self, config: dict):
        ...
        lstm = layers.Bidirectional(layers.LSTM(config['cells'], kernel_regularizer=weight_reg, recurrent_regularizer=weight_reg))(attention)
